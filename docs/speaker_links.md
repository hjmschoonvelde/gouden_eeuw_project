...
The [registry](../data/metadata/speaker_profiles.json) contains one entry per person and
...
the [metadata corrections](metadata_corrections.md). The dashboard loader and the research repository's metadata checks reject
...
[government metadata review](government_metadata.md) verifies and fills those
